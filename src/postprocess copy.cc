...
#define LABEL_NALE_TXT_PATH "../model/railway_11_labels_list.txt"
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                if (j >= x1 && j < x2 && i >= y1 && i < y2)
                {
                    ...
                    //     }
                    // }
                    // 通过指定类别控制最终只输出railway类别的mask
                    // 对于本例，只需要获取railway的准确mask
                    // 因为它存在远小近大的问题，若直接使用检测框误差很大
                    // 这也是需要使用seg而不是detect的原因
                    // 至于其他类别的物体，在图像中占有的区域很小
                    // 因此可以近似的使用该物体的检测框来代表该物体的位置
                    // 进一步判断该物体与railway mask的相对位置关系
                    // 来确定该物体是否处于railway范围内
                    if (all_mask_in_one[i * width + j] == 0 && cls_id[b] == 0)
                    {
...
void matmul_by_npu_fp(std::vector<float> &A_input, float *B_input, uint8_t *C_input, int ROWS_A, int COLS_A, int COLS_B, rknn_app_context_t *app_ctx)
{
    int B_layout = 0;
    int AC_layout = 0;
    int32_t M = ROWS_A;
...
{
    printf(" start process the outputs of rknn");
    ...
    printf(" finished process the outputs of rknn");
    ...
    // for(auto result : od_results->results){
    //     std::cout << " result id = " << result.cls_id << std::endl;
    // }
    ...
    crop_mask_uint8(matmul_out, all_mask_in_one, filterBoxes_by_nms, boxes_num, cls_id, proto_height, proto_width);
    ...
    //             model_in_h, model_in_w, cropped_height, cropped_width, ori_in_height, ori_in_width, y_pad, x_pad);
